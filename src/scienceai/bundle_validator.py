...
try:
    with open(os.path.join(path_to_app, "bundle_validator_prompt.txt"), "r") as f:
        ...
        try:
            ...
            for col in df.columns:
                if df[col].dtype == 'object':
                    ...
                    if mask.any():
                        count = mask.sum()
                        results.append(f"  - [{rel_path}] Column '{col}' has {count} rows containing 'failed_extraction' text")
...
def _run_validator_agent(workspace, file_list, original_zip_path):
    ...
    try:
        ...
        failed_extraction_scan = f"**Failed extraction scan error:** {str(e)}"
    
    try:
        structure_check = _check_bundle_structure(workspace)
    except Exception as e:
        structure_check = f"**Structure check error:** {str(e)}"
    
    try:
        effect_direction_check = _check_effect_direction_consistency(workspace)
    except Exception as e:
        effect_direction_check = f"**Effect direction check error:** {str(e)}"
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "summary": {
                            "type": "string",
                            "description": "Brief summary of the validation for the PI"
                        }
                    },
...
Call report_validation_result when done."""
    ...
    print(f"[Bundle Validator] Starting validation loop...")
        ...
        try:
            ...
            if not assistant_message.tool_calls:
                ...
                if consecutive_no_tools >= MAX_NO_TOOL_RESPONSES:
                    print(f"[Bundle Validator] Too many no-tool responses, forcing completion request")
                    messages.append({
                        "role": "user", 
            ...
            for tool_call in assistant_message.tool_calls:
                ...
                if function_name == "run_python_code":
                    ...
                    messages.append({
                        "role": "tool",
                        "tool_call_id": tool_call.id,
                        "content": "Validation result recorded."
                    })
            ...
            messages.append({
                "role": "user",
                "content": f"Error occurred: {str(e)}. Please continue or report your findings."
            })
    
    if not validation_complete:
        print(f"[Bundle Validator] Did not complete after {iteration} iterations")
        return ("## ⚠️ BUNDLE VALIDATION INCOMPLETE\n\n"
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "zip_path": {
                        "type": "string",
                        "description": "Path to the analysis bundle zip file to validate"
                    }
                },
                "additionalProperties": False,
                "required": ["zip_path"]
